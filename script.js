var playTurn, move, isGameOver, span, restartBtn;
playTurn = "X";
move = 0;
isGameOver = false;
...
function play(y) {
  if (y.dataset.player == "none" && isGameOver == false) {
    y.innerHTML = playTurn;
    y.dataset.player = playTurn;
    move++;
...
function checkWinner(a, b, c) {
  ...
  b--;
  c--;
  if ((span[a].dataset.player === span[b].dataset.player && span[b].dataset.player === span[c].dataset.player && span[a].dataset.player !== "none") && isGameOver === false) {
...
function gameOver(a) {
  var gameOverAlertElement = "<b>GAME OVER</b><br><br> Player " + span[a].dataset.player + " Wins!!! <br><br>" + restartBtn;
...
function draw() {
  var drawAlertElement = '<b>DRAW !!!</b>' + restartBtn;
  var div = document.createElement("div");
  div.className = "alert";
